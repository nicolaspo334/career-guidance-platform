...
import { centerChangePassword, isCenterLoggedIn } from '@/lib/api';
...
export default function CambiarContrasenaCentro() {
  ...
  useEffect(() => {
    if (!isCenterLoggedIn()) router.replace('/licencias');
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    if (newPass === current) { setError('La nueva contraseña debe ser distinta a la actual'); return; }
    setLoading(true);
    setError('');
    try {
      await centerChangePassword(current, newPass);
    ...
    <div className="e-page" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 20px', minHeight: '100vh' }}>
      ...
      <div className="e-card" style={{ width: '100%', maxWidth: 400, padding: 28, boxShadow: '0 4px 24px rgba(20,21,43,0.08)' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'grid', gap: 16 }}>
          <div>
            <label className="e-label">Contraseña temporal (recibida por email)</label>
            <input type="password" value={current} onChange={(e) => setCurrent(e.target.value)}
              required placeholder="Contraseña del email" className="e-input"/>
          </div>
