...
async function requireCenter(req, env) {
  const token = getToken(req);
  if (!token) return null;
  const payload = await verifyJWT(token, env.JWT_SECRET);
  if (!payload || payload.role !== 'center') return null;
  return payload;
}
...
async function checkRateLimit(env, ip, email) {
  ...
  ).bind(ip, email, cutoff).all();
  return results[0]?.count >= MAX_LOGIN_ATTEMPTS;
}

async function recordFailedAttempt(env, ip, email) {
  await env.DB.prepare(
    'INSERT INTO login_attempts (id, ip, email) VALUES (?, ?, ?)'
  ).bind(generateId(), ip, email).run();
}

async function clearAttempts(env, ip, email) {
  await env.DB.prepare(
  ...
  async fetch(req, env) {
    ...
    try {
      ...
      if (path === '/api/admin/login' && method === 'POST') {
        ...
        if (await checkRateLimit(env, ip, 'admin')) {
          return err(`Demasiados intentos. Espera ${LOCKOUT_MINUTES} minutos.`, 429);
        }
        ...
        if (!valid) {
          await recordFailedAttempt(env, ip, 'admin');
      ...
      if (path === '/api/admin/licenses' && method === 'GET') {
        ...
        const { results } = await env.DB.prepare(
          `SELECT l.*, c.name as center_name, c.email as center_email,
           ...
           FROM licenses l JOIN centers c ON c.id = l.center_id
      ...
      if (path === '/api/center/login' && method === 'POST') {
        ...
        if (!licenseCode) return err('Código de licencia requerido');

        const license = await env.DB.prepare(
          `SELECT l.*, c.name as center_name FROM licenses l
           JOIN centers c ON c.id = l.center_id
           WHERE l.code = ?`
        ).bind(licenseCode.toUpperCase()).first();

        if (!license) return err('Código de licencia no válido', 401);
        if (license.revoked) return err('Esta licencia ha sido revocada', 403);

        const token = await signJWT(
          { role: 'center', licenseId: license.id, centerName: license.center_name,
            exp: Math.floor(Date.now() / 1000) + 60 * 60 * 8 },
          env.JWT_SECRET
        );
        return json({ ok: true, token, centerName: license.center_name });
      }

      // ── CENTER: INFO ───────────────────────────────────────────────────────

      if (path === '/api/center/info' && method === 'GET') {
        const center = await requireCenter(req, env);
      ...
      if (path === '/api/center/users' && method === 'GET') {
        ...
        if (!center) return err('No autorizado', 401);
        const { results } = await env.DB.prepare(
          `SELECT id, name, email, created_at, active FROM users
      ...
      if (path === '/api/center/users' && method === 'POST') {
        const centerPayload = await requireCenter(req, env);
        if (!centerPayload) return err('No autorizado', 401);
        const { name, email } = await req.json();
        if (!name || !email) return err('Faltan campos');
        const result = await createUserForLicense(env, { name, email, licenseId: centerPayload.licenseId });
        if (result.error) return err(result.error, result.status || 400);
        return json({ ok: true, ...result });
      }
      ...
      if (path === '/api/center/users/deactivate' && method === 'POST') {
        ...
        // Verify user belongs to this license
        const user = await env.DB.prepare(
          'SELECT id FROM users WHERE id = ? AND license_id = ?'
      ...
      if (path === '/api/center/users/reactivate' && method === 'POST') {
        ...
        if (activeUsers.length >= license.max_users) {
          return err('Licencia sin plazas disponibles');
        }
        const user = await env.DB.prepare(
          'SELECT id FROM users WHERE id = ? AND license_id = ?'
        ).bind(userId, center.licenseId).first();
      ...
      if (path === '/api/auth/register' && method === 'POST') {
        ...
        if (!license) {
          await recordFailedAttempt(env, ip, email);
          return err('Código de licencia no válido', 400);
        }
        ...
        if (activeUsers.length >= license.max_users) {
          return err('Esta licencia no tiene plazas disponibles', 400);
        }
        ...
        const token = await signJWT(
          { role: 'user', userId, name, email, exp: Math.floor(Date.now() / 1000) + 60 * 60 * 24 },
          env.JWT_SECRET
        );
      ...
      if (path === '/api/auth/login' && method === 'POST') {
        ...
        const license = await env.DB.prepare(
          'SELECT revoked FROM licenses WHERE id = ?'
        ).bind(user.license_id).first();
...
async function createUserForLicense(env, { name, email, licenseId }) {
  const license = await env.DB.prepare(
    'SELECT * FROM licenses WHERE id = ? AND revoked = 0'
  ...
  if (activeUsers.length >= license.max_users) {
    return { error: 'Licencia sin plazas disponibles' };
  }
